...
Any special logic, variables and formatting instructions  are wrapped in `#{....}` blocks.
...
In this case, the part wrapped in `#{...}` will be evaluated and the result will be replaced back in the text.
...
The following values evaluate to `false`:
    ...
    * `null`
    * `false`
    * `"false"` (the text 'false')

Other values evaluate to `true`

* `and` - Example: `#{firstvar? and secondvar?}` - Returns true if both firstvar and secondvar are true
* `or` - Example: `#{firstvar? or secondvar?}` - Returns true if any of firstvar and secondvar are true
* `not` - negates the experssion proceding it - Example: `#{not firstvar?}` - Returns false if firstvar is true
...
This evaluates to `secondvar` if `firstvar` is true, else it evalutes to `thirdvar`
...
If the `authorized` variable is true then evaluate to the text "Ok" else evaluate to the text "Sorry, no access permitted"
...
Checks if `enable` is passed as a query string parameter and is set to some value except 0 or empty and then returns "OK" or "No" accordingly.
...
Where `app` is the application name and `rolename` is the name of the role to check
...
Where `FieldName` can be any  of the following:
